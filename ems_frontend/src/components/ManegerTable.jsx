...
const ManagerTable = () => {
  ...
  useEffect(() => {
    dispatch(
      getAllEmployeesByRole("MANAGER", searchQuery, selectedDepartment)
    );
  ...
  useEffect(() => {
    if (message) {
      const timer = setTimeout(() => {
        ...
        dispatch(getAllEmployeesByRole("MANAGER"));
      }, 3000);
          ...
          <thead className="bg-gray-50 dark:bg-gray-700">
            <tr>
              ...
              {role === ROLES.ROLE_ADMIN &&
                authorities.includes(PERMISSIONS.MANAGER_READ) && (
                  <th className="py-3 px-6 text-left text-xs font-semibold text-gray-700 dark:text-gray-300">
                    Email Address
                  </th>
                )}
          ...
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            {employees?.map((employee, index) => (
              <tr key={index}>
                ...
                {role === ROLES.ROLE_ADMIN &&
                  authorities.includes(PERMISSIONS.MANAGER_UPDATE) &&
                  authorities.includes(PERMISSIONS.MANAGER_UPDATE) && (
                    <td className="py-4 px-6 text-right">
                      <div className="flex space-x-2 justify-end">
                        <button
                          onClick={() =>
                            navigate(`/update-manager/${employee?.id}`)
                          }
